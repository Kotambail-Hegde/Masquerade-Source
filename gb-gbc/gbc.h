...
private:
	...
	typedef struct
	{
		...
		// NOTE: dead under the direct-write design below -- push sites now resolve
		// and write the final Pixel in one step, no separate raw+resolve pass. Safe
		// to delete once the pixel-push edits (#5 below) are applied.
		...
		// 4 SGB background palettes, 4 colors each (RGB555). Directly active/visible --
		// distinct from the 512-entry PAL_TRN pool (deferred, not yet implemented).
		// Per gbdev.io/pandocs/SGB_Command_Palettes.html PAL01/PAL23/PAL03/PAL12.
		uint16_t sgbBGPalettes[4][4];

		// Per-8x8-tile palette assignment (0-3) for the 20x18 GB window.
		// [row][col], row 0-17, col 0-19. Per
		// gbdev.io/pandocs/SGB_Command_Attribute.html ATTR_BLK/LIN/DIV/CHR.
	...
	MASQ_INLINE Pixel resolveSGBPixel(uint8_t colorId, uint8_t screenX, uint8_t screenY)
	{
		...
		uint8_t paletteNum = pGBc_display->sgbAttributeMap[tileY][tileX];
		uint16_t rgb15 = pGBc_display->sgbBGPalettes[paletteNum][colorId];
	...
	MASQ_INLINE void handleSGB_PAL_XX(uint8_t commandCode, const uint8_t* packetData)
	{
		...
		pGBc_display->sgbBGPalettes[paletteA][0] = sharedColor0;
		pGBc_display->sgbBGPalettes[paletteB][0] = sharedColor0;

		pGBc_display->sgbBGPalettes[paletteA][1] = readColor(3);
		pGBc_display->sgbBGPalettes[paletteA][2] = readColor(5);
		pGBc_display->sgbBGPalettes[paletteA][3] = readColor(7);

		pGBc_display->sgbBGPalettes[paletteB][1] = readColor(9);
		pGBc_display->sgbBGPalettes[paletteB][2] = readColor(11);
		pGBc_display->sgbBGPalettes[paletteB][3] = readColor(13);

		LOG("[SGB] PAL_XX Executed: Loaded palettes %d and %d", paletteA, paletteB);
	}
	...
	MASQ_INLINE void handleSGB_ATTR(uint8_t commandCode, const uint8_t* packetData)
	{
			...
			for (uint8_t set = 0; set < numDataSets; ++set)
			{
				...
				for (int y = 0; y < 18; ++y)
				{
					for (int x = 0; x < 20; ++x)
					{
						...
						{
							pGBc_display->sgbAttributeMap[y][x] = linePalette;
						}
						else if (inRect && !onBorder && changeInside)
						{
							pGBc_display->sgbAttributeMap[y][x] = insidePalette;
						}
						else if (!inRect && changeOutside)
						{
							pGBc_display->sgbAttributeMap[y][x] = outsidePalette;
						}
					}
				}
			}
			LOG("[SGB] ATTR_BLK Executed: %d data set(s)", numDataSets);
			BREAK;
		}
		...
		case SGB_COMMAND::SGB_CMD_ATTR_LIN:
		{
			...
			for (uint8_t set = 0; set < numDataSets; ++set)
			{
				...
				if (horizontal && lineNumber < 18)
				{
					for (int x = 0; x < 20; ++x)
					{
						pGBc_display->sgbAttributeMap[lineNumber][x] = palette;
					}
				}
				else if (!horizontal && lineNumber < 20)
				{
					for (int y = 0; y < 18; ++y)
					{
						pGBc_display->sgbAttributeMap[y][lineNumber] = palette;
					}
				}
			}
			LOG("[SGB] ATTR_LIN Executed: %d data set(s)", numDataSets);
			BREAK;
		}
		...
		case SGB_COMMAND::SGB_CMD_ATTR_DIV:
		{
			...
			for (int y = 0; y < 18; ++y)
			{
				for (int x = 0; x < 20; ++x)
				{
					...
					if (pos < coord)
					{
						pGBc_display->sgbAttributeMap[y][x] = paletteAboveLeft;
					}
					else if (pos > coord)
					{
						pGBc_display->sgbAttributeMap[y][x] = paletteBelowRight;
					}
					else
					{
						pGBc_display->sgbAttributeMap[y][x] = paletteDivLine;
					}
				}
			}
			LOG("[SGB] ATTR_DIV Executed: %s split at %d", horizontalSplit ? "horizontal" : "vertical", coord);
			BREAK;
		}
		...
		case SGB_COMMAND::SGB_CMD_ATTR_CHR:
		{
			...
			for (uint16_t i = 0; i < numDataSets; ++i)
			{
				...
				{
					pGBc_display->sgbAttributeMap[y][x] = palette;
				}
			...
			LOG("[SGB] ATTR_CHR Executed: %d data set(s), start (%d,%d)", numDataSets, beginX, beginY);
			BREAK;
		}
	...
	MASQ_INLINE void resolvePendingSGBTrnIfDue()
	{
		...
		// Border is only redrawn when its data actually changed -- not every
		// frame. Live GB pixels are written directly by the pixel push and
		// never touch this region, so nothing else needs to redraw it.
